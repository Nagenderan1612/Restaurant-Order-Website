...
from pic.models import Contact, Dish, Team, Category, Profile, Order
...
def contact_us(request):
    ...
    if request.method=="POST":
        ...
        obj.save()
        context['message']=f"Dear {name}, Thanks For Your Precious Time!"

    return render(request,'contact.html', context)

def team_members(request):
    """
    View function to display team members.
    """
    context = {}
...
def all_dishes(request):
    """
    View function to display all dishes.
    """
    context = {}
    dishes = Dish.objects.all()

    # Filter dishes by category if query parameter 'q' is present
    if 'q' in request.GET:
